...
public class Xml {
     ...
     * 
     * 
     * @return XmlTag-Array das die konvertierten Daten enth�lt
     */
    public void parseXml() throws Exception {
        ...
        do {
            // den n�chsten Tag abholen & hinzuf�gen:
            try {
     ...
     * <p/>
     * Sucht sich im Xml-Quelltext den n�chsten Tag und l�st diesen in ein
     * Tag-Objekt auf.
     */
    private Xml parseNextXmlTag() throws Exception {
        ...
        // pr�fen, ob �berhaupt ein Tag vorliegt:
        if (startPoint == -1) {
        ...
        // pr�fen, ob das tag valid ist, indem gepr�ft wird, ob bereits ein
        // neues tag ge�ffnet wurde
        ...
        if (endTagPoint == -1) {
            // Tag ist nicht vern�ftig geclosed worden
            throw new Exception(Const.ERROR_XMLFAILURE);
        }
        ...
        // sonderfall javascript ber�cksichtigen:
        int nextTag;
    ...
    /*
     * @author Tobias Janssen F�gt dem XmlArray das �bergebene xml-tag zu.
     * 
     * @param tag XmlTag, das dem array hinzugef�gt werden soll
     * 
     * @return XmlTag-Array mit der XmlStruktur
     */
    private Xml[] AddTagToArray(Xml[] array, Xml tag) {
        // pr�fen ob ein Schlie�er Tag vorliegt

        if (tag.isEndTag == true) {
            Xml backupTag = this.clone();
            Boolean breakout = false;
            // den �ffner von diesem Tag suchen und auf open=false setzen
            do {
                if (currentTag.type.equalsIgnoreCase(tag.type) && currentTag.isEndTag == false) {
                    // �ffner element gefunden, nun schlie�en
        ...
        // ansonsten muss das Element an den letzten offenen tag angef�gt werden
        else if (array.length > 0) {
            // den letzten offenen tag finden

            if (currentTag.open == true && currentTag.isEndTag == false) {
                // �ffner element gefunden, nun hinzuf�gen
    ...
    /*
     * @author Tobias Janssen F�gt den Parametern ein weteres Element hinzu
     * 
     * @param name String Bezeichnung f�r den Parameter
     * 
     * @param value String Value f�r den Parameter
     */
