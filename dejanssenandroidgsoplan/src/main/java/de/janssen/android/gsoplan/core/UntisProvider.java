...
import de.janssen.android.gsoplan.dataclasses.Lesson;
import de.janssen.android.gsoplan.dataclasses.SelectOptions;
import de.janssen.android.gsoplan.service.HttpSession;
...
public class UntisProvider {
    ...
    private static boolean compareEvents(ICalEvent existing, ICalEvent newEvent) {
        ...
        if (!existing.LOCATION.equalsIgnoreCase(newEvent.LOCATION))
            return true;
        if (!existing.SUMMARY.equalsIgnoreCase(newEvent.SUMMARY))
            return true;
