...
import de.ribeiro.android.gso.activities.MainActivity;
...
import de.ribeiro.android.gso.core.Stupid;
...
public class BackgroundSync extends AsyncTask<Boolean, Integer, Boolean> {
            ...
            if (proceed) {
                ...
                try {
                    if (now.getTimeInMillis() - this.mProfil.mylastResync >= this.mProfil.myResync * 1000 * 60 || fromFrontend) {
                        ...
                        if (!mProfil.onlyWlan || ctxt.isWifiConnected()) {
                            try {
                                _logger.Info("---------------------------------");
    ...
    private void syncAllDataInBackground(HtmlResponse htmlResponse) throws Exception {
