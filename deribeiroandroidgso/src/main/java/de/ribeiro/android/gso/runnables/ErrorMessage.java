 ...
 */
package de.ribeiro.android.gso.runnables;
...
public class ErrorMessage implements Runnable {
    private static Logger _logger = new Logger(Const.APPFOLDER, "ErrorMessage");
     ...
     * @param errorMessage     String der die Meldung enth�lt
     * @param onClick          OnClickListener f�r positiven Klick
    ...
    public void run() {
        if (ctxt.mIsRunning && errorMessage != null && !errorMessage.equalsIgnoreCase("")) {
            try {
                ...
                _logger.Error("Cannot create Dialog Interface", e);
            }
        }
    }
}
