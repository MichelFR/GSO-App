...
import de.ribeiro.android.gso.core.Profil;
...
public class Convert {
    ...
    /*for (int y = 0; y < data.timetable.length; y++)
	{
        ...
        result += Ical.Export(data.events);
        result += "\t</timetable>\n</week>\n";
     ...
     * @author Tobias Janssen
     */
    private static WeekData toWeekData(Xml xmlTimeTableTag) throws ParseException {
...
//		Calendar cal = new GregorianCalendar();
//		cal.setTimeInMillis(Long.parseLong(week.getParameterAtIndex(i).getValue()));
                weekData.date = cal;
            }
        ...
        weekData.events = Ical.Parse(xmlTimeTableTag.getDataContent());
    ...
    public static String toXml(List<Profil> profiles) {
        ...
        for (int i = 0; i < profiles.size(); i++) {
            ...
            result += "\t\tfastLoad='" + profiles.get(i).fastLoad + "'\n";
            result += "\t\tmuteEvents='" + profiles.get(i).muteEvents + "'\n";
            result += "\t>\n";
            result += "\t</" + Xml.PROFIL + ">\n";
        }
        result += "</" + Xml.PROFILES + ">\n";
    ...
    public static List<Profil> toProfiles(Xml xml, MyContext ctxt) {
        ...
        int myTypeIndex;
        Boolean onlyWlan, hideEmptyHours, autoSync, notificate, vibrate, sound, fastLoad, muteEvents;
        ...
        for (int ci = 0; ci < xml.getChildTags().length; ci++) {
            ...
            for (int i = 0; i < child.getParameters().length; i++) {
                ...
                if (child.getParameterAtIndex(i).getName().equalsIgnoreCase("onlyWlan")) {
                    onlyWlan = child.getParameterAtIndex(i).getValue().equalsIgnoreCase("true");
                }
                if (child.getParameterAtIndex(i).getName().equalsIgnoreCase("hideEmptyHours")) {
                    hideEmptyHours = child.getParameterAtIndex(i).getValue().equalsIgnoreCase("true");
                }
                if (child.getParameterAtIndex(i).getName().equalsIgnoreCase("autoSync")) {
                    autoSync = child.getParameterAtIndex(i).getValue().equalsIgnoreCase("true");
                }
                if (child.getParameterAtIndex(i).getName().equalsIgnoreCase("notificate")) {
                    notificate = child.getParameterAtIndex(i).getValue().equalsIgnoreCase("true");
                }
                if (child.getParameterAtIndex(i).getName().equalsIgnoreCase("vibrate")) {
                    vibrate = child.getParameterAtIndex(i).getValue().equalsIgnoreCase("true");
                }
                if (child.getParameterAtIndex(i).getName().equalsIgnoreCase("sound")) {
                    sound = child.getParameterAtIndex(i).getValue().equalsIgnoreCase("true");
                }
                ...
                if (child.getParameterAtIndex(i).getName().equalsIgnoreCase("fastLoad")) {
                    fastLoad = child.getParameterAtIndex(i).getValue().equalsIgnoreCase("true");
                }
                if (child.getParameterAtIndex(i).getName().equalsIgnoreCase("muteEvents")) {
                    muteEvents = child.getParameterAtIndex(i).getValue().equalsIgnoreCase("true");
                }
            }

            Profil profil = new Profil(myElement, myTypeIndex, myTypeKey, myTypeName, onlyWlan, hideEmptyHours, autoSync,
                    notificate, vibrate, sound, myResync, mylastResync, fastLoad, muteEvents, ctxt);
            profiles.add(profil);
